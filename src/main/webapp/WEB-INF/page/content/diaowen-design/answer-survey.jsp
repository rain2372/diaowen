...
/* 				var data="stridlist="+stridlist+"&strvallist="+strvallist;
	...
	*/
	...
	function getSumTotal(sv, sna) {
		var tds = sna.substr(0, sna.length - 32);
		var selectgroup = $("select[name^=" + tds + "]");
		var sum = 0;
		selectgroup.each(function(i) {
			sum = sum * 1 + $(this).val() * 1;
		})

		var textreadonly = $("input:text[name^=" + tds + "]");
		//alert(typeof(textreadonly));
		if(typeof(textreadonly) == "undefined"){
			//alert('undefined');
		}else{
			//alert();
			textreadonly.val(sum);
		}
								...
								<div class="surveyQuItemContent">
									...
									<div class="quCoItem">
										...
										<table class="quCoChenTable" >
												<tr><td></td>
													<c:forEach items="${en.columns }" var="columnItem">
														<td class="quChenColumnTd"><label class="editAble quCoOptionEdit">
														<c:if test="${columnItem.optionName eq 'sum'}">
														总计：
														</c:if>
														<c:if test="${columnItem.optionName ne 'sum'}">
														${columnItem.optionName}
														</c:if>
														</label></td>
													</c:forEach>
												</tr>
												...
												<tr class="dwQuCoChenRowTr"><td class="quChenRowTd"><label class="editAble quCoOptionEdit">${rowItem.optionName }</label>
													<input type="hidden" name="item_qu_${en.quType }_${en.id }_${rowItem.id }"  value="cs_item_qu_${en.quType }_${en.id }_${rowItem.id }_" />
														...
														<input type="text" readonly="readonly" size="4" name="cs_item_qu_${en.quType }_${en.id }_${rowItem.id }_${columnItem.id}" > 
